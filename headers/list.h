...
template <class T>
	...
	public:
		...
		List(int _maxSize = 1, _bucketSize = 10, bool suppressWarning = false) :maxSize(_maxSize), bucketSize(_bucketSize) {
			if (!suppressWarning && _bucketSize == 0) cout << "WARNING: List's bucket size was set to 0. The List is immutable (List object at " << this << " )" << endl;
			arr = new T[_maxSize];
		};
		...
		void insert(T* elem, int pos = 0) {
			...
			if (size >= maxSize) {
				...
				memcpy(&alloc, &data, sizeof(T) * size);
				T* tmp = arr;
				arr = alloc;
				delete[] tmp;
			}
		...
		void remove(int pos) {
			...
			memcpy(arr, arr + sizeof(T) * (pos + 1), sizeof(T) * (size - pos - 1))
				if (size < maxSize - bucketSize) {
					maxSize -= 10;
					T* alloc = new T[maxSize];
					memcpy(&alloc, &data, sizeof(T) * maxSize);
					T* tmp = arr;
					arr = alloc;
					delete[] tmp;
				}
		...
		void push_back(T* elem) insert(elem, size);
		void pop_back() remove(size-1);
		void insert(T elem, int pos) insert(new T(elem), pos);
		void push_back(T elem) insert(new T(elem), size);
	};
